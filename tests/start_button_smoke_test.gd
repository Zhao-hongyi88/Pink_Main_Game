...
func _verify_standalone_hover_rotation() -> void:
	...
	assert(start_image.position.is_equal_approx(Vector2(66.0, 109.0)))
	assert(start_image.size.is_equal_approx(Vector2(100.0, 48.0)))
	assert(gear_back.get_index() < gear_front.get_index())
	assert(gear_front.get_index() < start_image.get_index())
	for texture_rect: TextureRect in [gear_back, gear_front, start_image]:
